...
import { Calendar, BookOpen, CheckCircle, Clock, Users, Plus, Star, MessageSquare, TrendingUp, Award, Book, X, Link as LinkIcon, ExternalLink, Copy, Edit, Image, Video, Presentation } from 'lucide-react';
import { User, Training } from '../types';
import TrainingEditor from './TrainingEditor';
import PresentationsView from './PresentationsView';
import EducationLogsView from './EducationLogsView';
...
export default function EducationDashboard({ user }: EducationDashboardProps) {
    const [activeTab, setActiveTab] = useState<'trainings' | 'habits' | 'library' | 'presentations' | 'logs'>('trainings');
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="min-h-[500px]">
                ...
                {activeTab === 'presentations' && <PresentationsView user={user} isAdmin={true} />}
                {activeTab === 'logs' && <EducationLogsView user={user} isAdmin={true} />}
            </div>
        </div>
    );
}
