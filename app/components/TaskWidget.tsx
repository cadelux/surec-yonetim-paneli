...
export default function TaskWidget({ user }: TaskWidgetProps) {
    ...
    const [activeTab, setActiveTab] = useState<TabType>(user?.role === 'admin' ? 'sent' : 'received');
    ...
    const loadTasks = async () => {
        ...
        try {
            ...
            // 3. Get all users to map names
            const usersPromise = FirebaseStorage.getUsers();

            const [userTasks, unitTasks, sent, allUsers] = await Promise.all([userTasksPromise, unitTasksPromise, sentTasksPromise, usersPromise]);
                                                    ...
                                                    <div className="flex items-center justify-between">
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            {task.assignedToUserId && (
                                                                <span className="text-[10px] text-primary bg-primary/10 px-2 py-1 rounded font-medium flex items-center gap-1">
                                                                    <UserIcon size={12} />
                                                                    {userNames[task.assignedToUserId] || 'Bilinmiyor'}
                                                                </span>
                                                            )}
                                                            {task.readAt ? (
                ...
                <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200" style={{ zIndex: 100 }}>
                    <div onClick={(e) => e.stopPropagation()} className="bg-white text-zinc-900 w-full max-w-md rounded-3xl shadow-2xl border border-zinc-200 overflow-hidden animate-in zoom-in-95 duration-200">
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div className="bg-green-50 dark:bg-zinc-100 p-4 rounded-xl border border-green-100 dark:border-zinc-200">
                                <p className="text-xs mb-1 flex items-center gap-1 text-zinc-900 dark:text-zinc-900 font-semibold"><MessageCircle size={12} /> CEVAP</p>
                                <p className="text-sm text-zinc-900 dark:text-zinc-900 mt-1">{viewingFeedbackTask.completionNote}</p>
                            </div>
