...
import { BookOpen, CheckCircle, PlayCircle, Clock, Search, Award, ArrowRight, ExternalLink, Calendar, Presentation } from 'lucide-react';
...
export default function UserEducationView({ user }: UserEducationViewProps) {
    ...
    const [activeTab, setActiveTab] = useState<'browse' | 'my-learning' | 'presentations'>('browse');
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex bg-surface border border-border p-1 rounded-xl w-fit">
                    ...
                    <button
                        ...
                        className={clsx(
                            "px-4 py-2 rounded-lg text-sm font-bold transition-all flex items-center gap-2",
                            activeTab === 'presentations' ? "bg-primary text-primary-foreground shadow-sm" : "hover:bg-foreground/5 text-foreground/60"
                        )}
                    >
            ...
            {activeTab === 'presentations' && (
                <div className="pt-4">
                    <PresentationsView user={user} isAdmin={['admin', 'koordinator', 'sorumlu'].includes(user.role)} />
                </div>
            )}
        </div>
    );
