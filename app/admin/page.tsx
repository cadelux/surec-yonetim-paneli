...
import EducationLogsView from "../components/EducationLogsView";
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            ...
            const usersWithPasswords = data.map(u => {
                if (!u.password) {
                    const generatedPass = u.displayName.split(' ')[0].toLowerCase().trim().replace(/[^a-z0-9]/g, '');
...
function AnalysisView() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const [eData, uData, lData, pData, cData] = await Promise.all([
                FirebaseStorage.getEntries(),
                FirebaseStorage.getUsers(),
                FirebaseStorage.getEducationLogs(),
    ...
    return (
        <div className="space-y-10 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-surface/30 border border-border rounded-3xl p-6 space-y-6">
                    <div className="flex items-center justify-between">
                        <h3 className="text-sm font-bold uppercase tracking-wider text-foreground/50">Eğitim Verileri</h3>
                        <div className="p-2 w-8 h-8 flex items-center justify-center bg-blue-500/10 rounded-lg">
                            <BookOpen size={16} className="text-blue-500" />
                        </div>
                    </div>
                    <div className="space-y-4">
                        <div className="flex items-center justify-between p-3.5 bg-card border border-border rounded-xl hover:border-primary/30 transition-colors">
                            <span className="text-sm font-medium">Toplam Arşivlenen Eğitim</span>
                            <span className="font-bold text-lg text-primary">{logs.length}</span>
                        </div>
                        <div className="flex items-center justify-between p-3.5 bg-card border border-border rounded-xl hover:border-primary/30 transition-colors mb-2">
                            <span className="text-sm font-medium">Toplam Slayt & Sunum</span>
                            <span className="font-bold text-lg text-primary">{presentations.length}</span>
                        </div>
                        <div className="space-y-3 mt-4 pt-4 border-t border-border">
                            <p className="text-[10px] font-bold uppercase tracking-wider text-foreground/50 mb-4">Kategorilere Göre Sunumlar</p>
                            {categories.map(cat => {
                                const catCount = presentations.filter(p => p.category === cat.name).length;
                                return (
                                    <div key={cat.id} className="flex justify-between items-center text-sm">
                                        ...
                                        <span className="font-bold bg-surface px-2.5 py-1 rounded-md text-foreground/70">{catCount}</span>
                                    </div>
                                )
                            })}

                            {categories.length === 0 && (
                                <div className="text-xs text-foreground/40 text-center italic py-2">
...
const EducationManagementView = () => {
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            {/* Eğitim Kayıtları (Arşiv) Yöneticisi */}
            <hr className="my-8 border-border" />
            {user && <EducationLogsView user={user} isAdmin={true} />}
        </div>
    );
};
